...
function REQUEST_UPDATE ($cc_vars) {
	...
	//Provide access to our global variables in the config file.
	global $db_user, $db_pass, $db_host, $database, $sql, $err_msg, $num_rows, $result;
	global $GROUP_TABLE, $USER_TABLE, $REQUEST_TABLE, $VOTE_TABLE;
	...
	if ($isowner) {
		...
		if ($num_rows < 1){
			...
			$err_msg =  "success";
			//Reset the votes on this request
			$sql = "";
			$sql = "DELETE FROM `$VOTE_TABLE` WHERE requestid='$rid'";
			...
			if ($num_rows < 1){
				$err_msg =  "Error: unable to reset voting on the current request.";
			} else {
				$err_msg = "voting successfully reset.";
			} //end if
...
function SET_VOTE ($cc_vars) {
	...
	if ($has_voted == "false"){
		...
		if ($num_rows < 1){
			...
			//Build a query to insert a new request into the group queu.
			//Check to make sure a comment was entered first
			if($cc_vars['vcomment'] != '') {
				$sql = "";
